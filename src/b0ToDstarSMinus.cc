...
int main(int argc, char* argv[]) {
  ...
  for (int iEvent = 0; iEvent < maxEvents; ++iEvent) {
    ...
    for (int iPrt = 0; iPrt < (int)event.size(); ++iPrt) {
      ...
      //std::cout<<"B0 Meson Found : "<<event[iPrt].idAbs()<<"\n";
      for (int iDtr = event[iB0].daughter1(); iDtr <= event[iB0].daughter2(); ++ iDtr) {
        ...
        if (event[iDtr].idAbs() ==  413)  iDStar1= event[iDtr].iBotCopyId();
      }
      //std::cout<<"\t iB0 "<<iB0<<" D_s*- , mu , mu : "<<iDStar1<<", "<<iMu1<<", "<<iNuMu1<<"\n"; 
      if(iB0 ==-1 ) continue;
      if( iDStar1  ==-1 ) continue;
      if( iMu1  ==-1 ) continue;
      if( iNuMu1  ==-1 ) continue;
        
      //std::cout<<"\t  mu1 pt"<<event[iMu1].pT() <<" /  3.5 "<<"\n";
      if(event[iMu1].pT()  <  3.5 ) continue;
      if(event[iMu1].eta() < -2.5 ) continue;
      if(event[iMu1].eta() >  2.5 ) continue;
      
      for (int iDtr = event[iDStar1].daughter1(); iDtr <= event[iDStar1].daughter2(); ++ iDtr) {
        if (event[iDtr].idAbs() ==  411)  iD   = event[iDtr].iBotCopyId();
        if (event[iDtr].idAbs() ==  22 )  iG1  = event[iDtr].iBotCopyId();
      }
      //std::cout<<"\t\t iD ,  gamma "<<iD<<" , "<<iG1<<"\n"; 
      if( iD  ==-1 ) continue;
      if( iG1  ==-1 ) continue;
      //std::cout<<"\t  g1 pt"<<event[iG1].pT() <<" /  3.5 "<<"\n";
      if(event[iG1].pT() < 3.5 ) continue;
      if(event[iG1].eta() < -10000.0 ) continue;
      if(event[iG1].eta() > 3.0 ) continue;


      for (int iDtr = event[iD].daughter1(); iDtr <= event[iD].daughter2(); ++ iDtr) {
      ...
      treeManager.FillParticleBranches("D_Star1",event,iDStar1);
      treeManager.FillParticleBranches("Gamma",event,iG1);
      treeManager.FillParticleBranches("D",event,iD);
