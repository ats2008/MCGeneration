...
/grid_mnt/t3storage3/athachay/trippleHiggs/delphisStudies/MCGeneration/showerLHE_lepVeto.exe /grid_mnt/t3storage3/athachay/trippleHiggs/delphisStudies/MCGeneration/configs/lhe.cfg @@DECAY_FILE events.lhe events.hepmc3
...
zip events.lhe.zip events.lhe
if [ $? -eq 0 ]; then 
#    cp @@PTAG/Events/run_@@IDX/* @@DESTINATION
    cp *.zip *.root @@PTAG/Events/run_@@IDX/run*.txt @@DESTINATION
    if [ $? -eq 0 ] ; then
