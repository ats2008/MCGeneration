...
def main():
    ...
    localize_file=config['localize_file']
    if localize_file:
        print("Localizing file !! ")
    ...
    fileDict={}
    if isinstance(fileListIn,str):
        print(f"Opening file List from {fileListIn}")
        with open(fileListIn) as f:
            fileDict=json.load(f)
    else:
        print(f"Loading file List from config , with  {len(fileListIn)} procs")
        fileDict=fileListIn
    ...
    for proc in fileDict:
        ...
        for fileName in fileDict[proc]:
            ...
            if localize_file:
                cmd=f"cp {fileName} . " ;  print(f" []${cmd} ");os.system(cmd);
            ...
            try :
                with urt.open(fileName)  as f:
                    ...
                    if config['saveOutPut']:
                        ...
                        oFname=prefix+ fileName.split("/")[-1].replace('.root',f'_{proc}_{fIdx}_preselected.parquet')
                ...
                failedFiles[proc].append( fileName  )
                continue
            if localize_file:
                cmd=f"rm {fileName} . " ;  print(f" []${cmd} ");os.system(cmd);
    print("FailedFiles : ")
    print(json.dumps(failedFiles,indent=4))
    with open("failedFiles.json",'w') as f:
        json.dump(failedFiles,f)
